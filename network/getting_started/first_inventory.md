...
With the `-k` flag, you provide the SSH password(s) at the prompt. Alternatively, you can store SSH and other secrets and passwords securely in your group_vars files with `ansible-vault`. See network_vault for details.
...
The `--vault-id <ansible-playbook --vault-id>` flag allows different vault passwords for different users or different levels of access. The output includes the username `my_user` from your `ansible-vault` command and uses the YAML syntax `key: value`:
