...
To use the `netconf` connection plugin for network devices that support standard NETCONF (6241) operations such as `get`, `get-config`, `edit-config`, set `ansible_network_os=default`. You can use netconf_get, netconf_config and netconf_rpc modules to talk to a NETCONF enabled remote host.
...
- cliconf_plugins (for example, ios cliconf) - Provides an abstraction layer for low level send and receive operations. For example, the `edit_config()` method ensures that the prompt is in `config` mode before executing configuration commands.
