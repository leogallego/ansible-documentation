...
6.  Create a PR to the appropriate collection that you want to add your new resource module to. See contributing_maintained_collections for details on determining the correct collection for your module.
...
- `collection_org`: The organization of the collection, required when <span class="title-ref">structure=collection</span>.
- `collection_name`: The name of the collection, required when <span class="title-ref">structure=collection</span>.
...
You should run `ansible-test sanity` and `tox -elinters` from the collection root directory before pushing your PR to an Ansible-maintained collection. The CI runs both and will fail if these tests fail. See developing_testing for details on `ansible-test sanity`.

To install the necessary packages:

1.  Ensure you have a valid Ansible development environment configured. See environment_setup for details.
...
This example will run against all `vyos` modules. Note that `vyos_.*` is a regex match, not a bash wildcard - include the <span class="title-ref">.</span> if you modify this example.
...
If you need additional help or feedback, reach out to the community. Visit the Ansible communication guide for details.
...
See the unit test file [test_eos_l2_interfaces](https://github.com/ansible-collections/arista.eos/blob/main/tests/unit/modules/network/eos/test_eos_l2_interfaces.py) for a practical example.
