...
More information on how the Kerberos library attempts to find the KDC can be found in the [MIT Kerberos Documentation](https://web.mit.edu/kerberos/krb5-latest/doc/admin/realm_config.html).
...
To retrieve a TGT manually for a user, run the `kinit` command with the user's username and domain as shown in winrm_kerberos_verify_config. This TGT will be used automatically when Kerberos authentication is requested by the connection plugin in Ansible.
