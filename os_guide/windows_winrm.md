...
See windows_winrm_certificate for more information on how to configure and use certificate authentication.
...
Kerberos does require some additional setup work on the Ansible host before it can be used properly. See windows_winrm_kerberos for more information on how to configure, use, and troubleshoot Kerberos authentication.
...
The `openssl s_client` command will use the system trust store to validate the certificate which may not align with the trust store used in Ansible. See windows_winrm_cert_validation for more information.
