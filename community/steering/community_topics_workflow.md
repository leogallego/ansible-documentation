...
- By default, the discussion happens asynchronously in the topic.
  - A steering committee member can tag the forum post with `community-wg-nextmtg` to put it on the synchronous meeting agenda.
...
- Checks if there is a corresponding milestone in the [ansible-build-data](https://github.com/ansible-community/ansible-build-data/milestones) repository.
...
There are a few [scripts](https://github.com/ansible-community/community-topics/tree/main/scripts) that can be used to create Ansible community announcements on the Bullhorn and similar locations.
