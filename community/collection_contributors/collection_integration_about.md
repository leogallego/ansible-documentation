...
Integration tests check modules with playbooks that invoke those modules. The tests pass standalone parameters and their combinations, check what the module or plugin reports with the `assert <ansible.builtin.assert` module, and the actual state of the system after each task.
...
The names of targets contain the module or plugin name that they test. Target names that start with `setup_` are usually executed as dependencies before module and plugin targets start execution. See collection_creating_integration_tests for details.

To run integration tests, we use the `ansible-test` utility that is included in the `ansible-core` and `ansible` packages. See collection_run_integration_tests for details. After you finish your integration tests, see to collection_quickstart to learn how to submit a pull request.
...
1.  Register the outcome of the task as a variable, for example, `register: result`. Using the `assert <ansible.builtin.assert` module, check:

> 1.  If `- result is changed` or not.
> 2.  Expected return values.

2.  If the module changes the system state, check the actual system state using at least one other module. For example, if the module changes a file, we can check that the file has been changed by checking its checksum with the `stat <ansible.builtin.stat` module before and after the test tasks.
