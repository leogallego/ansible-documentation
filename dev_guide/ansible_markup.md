...
- `R()` for cross-references with a heading (supported since Ansible 2.10). For example: `See R(Cisco IOS Platform Guide,ios_platform_options)`. Use the RST anchor for the cross-reference. See adding_anchors_rst for details.
...
If you are creating your own documentation site, you will need to use the [intersphinx extension](https://www.sphinx-doc.org/en/master/usage/extensions/intersphinx.html) to convert `R()`, `M()`, `P()`, `O()`, and `RV()` to the correct links.
