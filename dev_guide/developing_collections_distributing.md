...
Before distributing your collection, ensure you have updated the `galaxy.yml` file. See collection_structure for details.
...
| Pulp 3 Galaxy All coll          | ections, supports signed collections                               |
...
- To create a namespace on Red Hat Automation Hub, see the [Red Hat automation content documentation](https://docs.redhat.com/en/documentation/red_hat_ansible_automation_platform/).

Specify the namespace in the `galaxy.yml` file for each collection. For more information on the `galaxy.yml` file, see collections_galaxy_meta.
...
- To get an API token for Galaxy, see [the Galaxy documentation](https://galaxy.ansible.com/ui/token/).
- To get an API token for Automation Hub, see the [Red Hat automation content documentation](https://docs.redhat.com/en/documentation/red_hat_ansible_automation_platform/).
...
You cannot use `apt-key` with any servers defined in your galaxy_server_list. See galaxy_server_config for complete details.
...
\* To reduce the size of collections, certain files and folders are excluded from the collection tarball by default. See ignoring_files_and_folders_collections if your collection directory contains other files you want to exclude. \* The current Galaxy maximum tarball size is 20 MB.
...
You can exclude files from your collection with either build_ignore or manifest_directives. For more information on the `galaxy.yml` file, see collections_galaxy_meta.
...
You can include a GnuPG signature with your collection on a Pulp 3 Galaxy server. See [Enabling collection signing](https://galaxyng.netlify.app/config/collection_signing/) for details.
...
This ansible-galaxy command assumes you have retrieved and stored your API token in configuration. See galaxy_specify_token for details.
...
See the [Galaxy documentation](https://ansible.readthedocs.io/projects/galaxy-ng/en/latest/usage_guide/collections/#via-the-ui) to learn how to publish your collection directly on the Galaxy website.
