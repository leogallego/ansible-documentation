...
Tests will be marked as unstable by a member of the Ansible Core Team. GitHub [issues](#issues) will be created to track each unstable test.
...
Tests will be marked as disabled by a member of the Ansible Core Team. GitHub [issues](#issues) will be created to track each disabled test.
...
Tests that are marked as [unstable](#unstable) or [disabled](#disabled) will have an issue created to track the status of the test. Each issue will be assigned to one of the following projects:
...
For questions about integration tests reach out to the community on the Ansible Forum by creating a topic tagged with `ansible-test`, `testing` and other appropriate tags.
