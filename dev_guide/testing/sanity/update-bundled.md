...
- A file has a \_BUNDLED_METADATA variable but the file isn't specified in `test/sanity/code-smell/update-bundled.py`. This typically happens when a new bundled library is added. Add the file to the <span class="title-ref">get_bundled_libs()</span> function in the <span class="title-ref">update-bundled.py</span> test script to solve this error.
