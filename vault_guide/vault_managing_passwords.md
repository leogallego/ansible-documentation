...
> - Pass it with `--vault-id <ansible-playbook --vault-id>` to the ansible-vault command when you create encrypted content
> - Include it wherever you store the password for that vault ID (see storing_vault_passwords)
> - Pass it with `--vault-id <ansible-playbook --vault-id>` to the ansible-playbook command when you run a playbook that uses content you encrypted with that vault ID
...
If your playbook uses multiple encrypted variables or files that you encrypted with different passwords, you must pass the vault IDs when you run that playbook. You can use `--vault-id <ansible-playbook --vault-id>` by itself, with `--vault-password-file <ansible-playbook --vault-password-file>`, or with `--ask-vault-pass <ansible-playbook --ask-vault-pass>`. The pattern is the same as when you create encrypted content: include the label and the source for the matching password.

See below for examples of encrypting content with vault IDs and using content encrypted with vault IDs. The `--vault-id <ansible-playbook --vault-id>` option works with any Ansible command that interacts with vaults, including ansible-vault, ansible-playbook, and so on.
