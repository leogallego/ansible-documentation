...
You can encrypt single values inside a YAML file using the ansible-vault encrypt_string command. For one way to keep your vaulted variables safely visible, see tip_for_variables_and_vaults.
...
To encrypt the string 'letmein' read from stdin, add the vault ID 'dev' using the 'dev' vault password stored in <span class="title-ref">a_password_file</span>, and name the variable 'db_password':
